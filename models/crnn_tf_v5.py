...
class CRNN(nn.Module):
    def __init__(self, num_classes, in_channels, in_channels_f, in_channels_s, model='rnn'):
        ...
        self.hidden_size = 32  # Define hidden size for RNN layers
        if self.rnn_type == 'lstm':
            self.rnn = nn.LSTM(input_size=74, hidden_size=self.hidden_size, num_layers=2, dropout=0.5, bidirectional=True)
        elif self.rnn_type == 'gru':
            self.rnn = nn.GRU(input_size=74, hidden_size=self.hidden_size, num_layers=2, dropout=0.5, bidirectional=True)
        elif self.rnn_type == 'rnn':
            self.rnn = nn.RNN(input_size=74, hidden_size=self.hidden_size, num_layers=2, dropout=0.5, bidirectional=True)
        else:
    ...
    def forward(self, x, x_freq, x_scl):
        ...
        # Residual connection: Adding the output before and after the RNN layer
        x4_residual = x4 + x3[:, -1, :self.hidden_size*2]  # Ensure the sizes match for addition

        # Attention mechanism
        attention_weights = self.softmax(self.attention(x4_residual))
        x4_weighted = torch.sum(x4_residual * attention_weights, dim=1)

        # Fully connected layer
        x4 = torch.cat([x4_weighted, torch.squeeze(x3)], dim=1)
