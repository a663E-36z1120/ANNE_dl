...
class CRNN(nn.Module):
    def __init__(self, num_classes, in_channels, in_channels_f, in_channels_s, model='rnn'):
        ...
        # Recurrent layers: 
        if model == 'lstm':
            self.rnn = nn.LSTM(input_size=48, hidden_size=18, num_layers=2,
                               bidirectional=True)
        elif model == 'gru':
            self.rnn = nn.GRU(input_size=36, hidden_size=18, num_layers=2,
                              bidirectional=True)
        elif model == 'rnn':
            self.rnn = nn.RNN(input_size=48, hidden_size=18, num_layers=2,
                              bidirectional=True)
        else:
        ...
        self.fc1 = nn.Linear(in_features=72, out_features=(32))
