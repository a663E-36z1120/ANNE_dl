...
from preprocessing.main import integrate_raw, integrate_data, augment_data
...
def train_model(model, optimizer, train_loaders, test_loaders, lr_scheduler, epochs=100, print_every=10):
    ...
    xentropy_weight = torch.tensor([1 / 30 ** 1.15, 1 / 35 ** 1.15, 1/15**1.15]).to(device)
    ...
    patience = 2000       # we early return if there is no improvement after patience number of epochs
    counter = 0
...
if __name__ == "__main__":
    ...
    for id in train_list:
        try:
            X, X_freq, X_scl, t = integrate_data(int(id), train_list[id])
            # t = np.where(t == 2, 1, t)
            print(f"{X.shape}, {X_freq.shape}, {X_scl.shape}")
            dataset = ANNEDataset(X, X_freq, X_scl, t, device)
            size = len(X)
            print(id)
            if int(id) not in validation_test_id_list:
                train_dataloaders.append(DataLoader(dataset=dataset, batch_size=size))
                if str(id) in white_list: # Prepare augmented dataset
                    print("generating augmented dataset...")
                    for delta in [2.85, 2.9, 2.95]:
                        X_raw, t = integrate_raw(int(id), train_list[id])
                        X_aug, X_freq_aug, X_scl_aug = augment_data(X_raw, delta)
                        print(f"{X_aug.shape}, "
                              f"{X_freq_aug.shape}, "
                              f"{X_scl_aug.shape}")
                        dataset_aug = ANNEDataset(X_aug, X_freq_aug, X_scl_aug, t, device)
                        train_dataloaders.append(DataLoader(dataset=dataset_aug, batch_size=size))
    ...
    scheduler = CyclicLR(optimizer, base_lr=0.00001, max_lr=0.0001, mode="triangular2", step_size_up=250, gamma=0.9, cycle_momentum=False)
    ...
    torch.save(model, "model_finetuned.pt")
    print("Model Saved")
